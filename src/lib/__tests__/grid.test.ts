...
describe('findFreeSlot (main grid)', () => {
  ...
  it('defaults missing w/h on existing links to 1×1', () => {
    ...
    const stub: WidgetItem = { ...link('a', 0, 0), w: undefined, h: undefined };
    const links: WidgetItem[] = [stub];
    // Existing link occupies just (0,0); next item goes to (1,0)
    expect(findFreeSlot(links, 1, 1)).toEqual({ x: 1, y: 0 });
  });
});

describe('findSlotInGroup', () => {
  it('places at (0,0) in an empty group', () => {
    expect(findSlotInGroup([], 1, 1)).toEqual({ x: 0, y: 0 });
  });
